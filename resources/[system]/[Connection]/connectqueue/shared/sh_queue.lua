...
Queue.JoinCbs = {}
Queue.TempPriority = {}
Queue.JoinDelay = GetGameTimer() + Config.JoinDelay and Config.JoinDelay or 0

-- EDIT THESE IN SERVER.CFG + OTHER OPTIONS IN CONFIG.LUA
Queue.MaxPlayers = 128
...
function Queue:DebugPrint(msg)
    if Queue.Debug then
        msg = "^3BlueVinity QUEUE Systeme: ^0" .. tostring(msg) .. "^7"
        print(msg)
    end
end
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(self.QueueList) do
        if tmp.priority then
            ...
            if _pos then
                self:DebugPrint(string_format("^3%s^9[^3%s^9] ^2was prioritized and placed ^1%d/%d ^2in queue", tmp.name, ids[1], _pos, queueCount))
                break
            end
        end
    end
    ...
    if not _pos then
        _pos = self:GetSize() + 1
        self:DebugPrint(string_format("^3%s^9[^3%s^9] ^2was placed ^1%d/%d ^2in queue", tmp.name, ids[1], _pos, queueCount))
    end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if Queue:NotFull(true) and Queue.JoinDelay <= GetGameTimer() then
        -- let them in the server
        local added = Queue:AddToConnecting(ids, true, true, done)
        if not added then CancelEvent() return end

        done()
        Queue:DebugPrint("^3"..name .. "^9[^3" .. ids[1] .. "^9] ^2is loading into the server")

        return
    end
    
    update(string_format(Config.Language.pos .. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " Prioritaire devant toi.)" or "00:00:00"), pos, Queue:GetSize(), ""))
    ...
    while true do
        ...
        if pos <= 1 and Queue:NotFull() and Queue.JoinDelay <= GetGameTimer() then
            -- let them in the server
            ...
            Queue:RemoveFromQueue(ids)
            Queue:DebugPrint("^3"..name .. "^0[^3" .. ids[1] .. "^0] ^2is loading into the server")
            return
        end
        ...
        local msg = string_format(Config.Language.pos .. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " Prioritaire devant toi.)" or ""), pos, Queue:GetSize(), qTime)
        update(msg, data.deferrals)
    end
end
