...
AddEventHandler('esx_vangelico_robbery:toofar', function(robb)
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			TriggerClientEvent('esx_vangelico_robbery:BraquageAnnule', xPlayers[i])
	...
	if(robbers[source])then
		...
		robbers[source] = nil
		TriggerClientEvent('esx_vangelico_robbery:BraquageAnnule')
	end
end)
...
AddEventHandler('esx_vangelico_robbery:endrob', function(robb)
	...
	for i=1, #xPlayers, 1 do
 		...
 		if xPlayer.job.name == 'police' then
			...
			TriggerClientEvent('esx_vangelico_robbery:killblip', xPlayers[i])
			TriggerClientEvent('esx_vangelico_robbery:BraquageFinit', xPlayers[i])
		end
	end
	if(robbers[source])then
	...
	SetTimeout(4000, function()
		...
		if PlayersCrafting[source] == true and CopsConnected >= Config.RequiredCopsSell then
			...
			if JewelsQuantity < 20 then 
				TriggerClientEvent('esx_vangelico_robbery:VenteBijouxRefuser', source)
			else   
				TriggerClientEvent('esx_vangelico_robbery:VenteBijoux', source)
