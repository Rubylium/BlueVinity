...
---------------------------------------------------------------------
function usePoliceSiren(veh)
	local model = GetEntityModel(veh)
	for i = 1, #eModelsWithPoliceSrn, 1 do
		if model == GetHashKey(eModelsWithPoliceSrn[i]) then
			return true
		end
	end
	return false
	...
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if newstate ~= state_lxsiren[veh] then
			...
			if newstate == 1 then
				...
				elseif usePoliceSiren(veh) then
					TogMuteDfltSrnForVeh(veh, false)
				else
	...
	if DoesEntityExist(veh) and not IsEntityDead(veh) then
		if toggle == true then
			if snd_pwrcall[veh] == nil then
				...
				if usePowercallAuxSrn(veh) or useFiretruckSiren(veh) then
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						if useFiretruckSiren(veh) and state_lxsiren[veh] == 1 then
							TogMuteDfltSrnForVeh(veh, false)
							dsrn_mute = false
						elseif usePoliceSiren(veh) and state_lxsiren[veh] == 1 then
							TogMuteDfltSrnForVeh(veh, false)
							dsrn_mute = false
						else
							TogMuteDfltSrnForVeh(veh, true)
						...
						if not IsPauseMenuActive() then
							...
							-- TOG DFLT SRN LIGHTS
							if IsDisabledControlJustReleased(0, 85) or IsDisabledControlJustReleased(0, 303) then
						...
						if hmanu_state_new == 1 then
							if not useFiretruckSiren(veh) or usePoliceSiren(veh) then
