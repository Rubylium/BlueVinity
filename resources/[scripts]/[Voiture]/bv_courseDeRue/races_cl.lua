...
AddEventHandler("StreetRaces:createRace_cl", function(index, amount, startDelay, startCoords, checkpoints)
    ...
    races[index] = race
    local blip = AddBlipForCoord(startCoords)
    SetBlipSprite(blip, 490)
    SetBlipColour(blip, 1)
    SetBlipScale(blip, 1.2)
    SetBlipPriority(blip, 50)
    local blip2 = AddBlipForCoord(startCoords)
    SetBlipSprite(blip2, 42)
    SetBlipColour(blip2, 1)
    SetBlipScale(blip2, 1.0)
    SetBlipPriority(blip2, 40)
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
    ESX.ShowAdvancedNotification("Course de rue", "~b~Une course de rue", "Une course de rue vas commencer ! Ramène toi avec ta caisse !", "CHAR_HAO", 8)
    Wait(120*1000)
    RemoveBlip(blip)
    RemoveBlip(blip2)
end)
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= config_cl.freezeDuration then
                    ...
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Course de rue pour ~g~$%d~w~ commence dans ~y~%d~w~s"):format(race.amount, math.ceil(count/1000.0)))
                    Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "Inscrit")
                end
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_cl.joinProximity and currentTime < race.startTime then
                        ...
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+1.0, ("Course de rue pour ~g~$%d~w~ commence dans ~y~%d~w~s"):format(race.amount, count))
                        Draw3DText(race.startCoords.x, race.startCoords.y, zCoord+0.80, "Appuyer sur [~g~E~w~] pour rejoindre")
