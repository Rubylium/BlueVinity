...
function LoadSellPlace()
	Citizen.CreateThread(function()
		...
		while true do
			...
			for i = 1, #ConfigVente.VehiclePositions, 1 do
				if ConfigVente.VehiclePositions[i]["entityId"] ~= nil then
					local pedCoords = GetEntityCoords(ped)
					local vehCoords = GetEntityCoords(ConfigVente.VehiclePositions[i]["entityId"])

					local dstCheck = GetDistanceBetweenCoords(pedCoords, vehCoords, true)
					local pPed = GetPlayerPed(-1)
					local VehName = GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(ConfigVente.VehiclePositions[i]["entityId"])))
					local vehicleClass = GetVehicleClass(ConfigVente.VehiclePositions[i]["entityId"])
					...
					if dstCheck <= 2.5 then
						...
						ESX.Game.Utils.DrawText3D(vehCoords, "~b~[E] ~w~Pour acheter.\nPrix: ~g~" .. ConfigVente.VehiclePositions[i]["price"] .. "$ \n~w~Modèle: ~g~"..VehName.."\n~w~Type: ~g~"..type.."", 0.8)

						if IsControlJustPressed(0, 38) then
							ESX.TriggerServerCallback('RubyMenu:getUsergroup', function(group)
								playergroup = group
								if playergroup == 'superadmin' or playergroup == 'owner' then
									if IsPedInVehicle(ped, ConfigVente.VehiclePositions[i]["entityId"], false) then
										OpenSellMenuStaff(ConfigVente.VehiclePositions[i]["entityId"], ConfigVente.VehiclePositions[i]["price"], true, ConfigVente.VehiclePositions[i]["owner"])
										print(ConfigVente.VehiclePositions[i]["vehicleProps"])
									else
										ESX.ShowNotification("Tu doit être dans le ~g~vehicule~s~!")
									end
								else
									if IsPedInVehicle(ped, ConfigVente.VehiclePositions[i]["entityId"], false) then
										OpenSellMenu(ConfigVente.VehiclePositions[i]["entityId"], ConfigVente.VehiclePositions[i]["price"], true, ConfigVente.VehiclePositions[i]["owner"])
									else
										ESX.ShowNotification("Tu doit être dans le ~g~vehicule~s~!")
									end
								end
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx-qalle-sellvehicles:isVehicleValid", function(valid)
				...
				if valid then
					...
					ESX.ShowNotification("Vous devez ~r~avoir~s~ le ~g~vehicule!~s~ ou trop de véhicule son en vente: " .. #ConfigVente.VehiclePositions .. " véhicules en ventes")
				end
...
--	local VehPos = ConfigVente.VehiclePositions
--
--	for i = 1, #VehPos, 1 do
