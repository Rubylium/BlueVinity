...
function OpenMobileMechanicActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_mechanic_actions', {
		...
		elements = {
			...
			{label = 'Annonce Mécano Ouvert',	value = 'mecano_ouvert'},
			{label = 'Annonce Mécano Fermé', 	value = 'mecano_ferme'},
		...
		elseif action == 'mecano_ouvert' then
			TriggerServerEvent('AnnounceMecanoOuvert')
		elseif action == 'mecano_ferme' then
		...
		elseif data.current.value == 'fix_vehicle' then
			...
			if DoesEntityExist(vehicle) then
				...
				Citizen.CreateThread(function()
					...
					--SetVehicleOilLevel(vehicle, 750)
