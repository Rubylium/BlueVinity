	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('tailer_clothes'), value = 'tailer_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'tailer_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'tailer_actions_menu'
...
function OpenTailerActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'tailer_actions',
		{
			title    = 'Couturier',
			align    = 'top-left',
			elements = elements
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'tailer', function(data, menu)
					menu.close()
				end)
			end
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'tailer', vehicleProps)
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint.Pos, 56.326, function(vehicle)
					...
					SetVehicleNumberPlateText(playerVeh, 'TAIL WORK')
				end)
			end,
			function(data, menu)
...
function OpenMobileTailerActionsMenu()

	ESX.UI.Menu.CloseAll()
	if PlayerData.job ~= nil and PlayerData.job.grade_name == 'boss' then
		ESX.UI.Menu.Open(
			'default', GetCurrentResourceName(), 'mobile_tailer_actions',
			{
				title    = 'Coututier',
				align    = 'top-left',
				elements = {
							...
							if amount == nil or amount <= 0 then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									Citizen.CreateThread(function()
										...
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_tailer', 'Couturier', amount)
									end)
								end
							end
						end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_tailerjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_tailerjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_tailerjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_tailerjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_tailerjob:hasEnteredMarker', function(zone)
	if zone == 'WoolFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'tailer'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementClothe' and PlayerData.job ~= nil and PlayerData.job.name == 'tailer'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement_clothe')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementTissu' and PlayerData.job ~= nil and PlayerData.job.name == 'tailer'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'tailer'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'TailerActions' and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
		CurrentAction     = 'tailer_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
...
AddEventHandler('esx_tailerjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'WoolFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
		TriggerServerEvent('esx_tailerjob:stopHarvest')
	end  
	if (zone == 'TraitementClothe' or zone == 'TraitementTissu') and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
		TriggerServerEvent('esx_tailerjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
		TriggerServerEvent('esx_tailerjob:stopSell')
	end
	CurrentAction = nil
end)

RegisterNetEvent('esx:tailerNotif')
AddEventHandler('esx:tailerNotif', function()
	TriggerEvent("pNotify:SetQueueMax", "right", 2)
	exports.pNotify:SendNotification({text = "Achats effectué, +1 laine. <b style='color:gray'>(<b style='color:red'>-2$<b style='color:gray'>)", type = "success", timeout = 2000, layout = "centerRight", queue = "right"})
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.TailerActions.Pos.x, Config.Zones.TailerActions.Pos.y, Config.Zones.TailerActions.Pos.z)
		...
		AddTextComponentString("Couturier")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'tailer' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_tailerjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_tailerjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_tailerjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_tailerjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_tailerjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_tailerjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'tailer_actions_menu' then
					OpenTailerActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'tailer', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'tailer' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileTailerActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)