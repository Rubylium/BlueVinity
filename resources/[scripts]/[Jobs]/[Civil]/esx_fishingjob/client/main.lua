	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('fishing_clothes'), value = 'fishing_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'fishing_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'fishing_actions_menu'
...
function OpenFishingActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'fishing_actions',
		{
			title    = 'Pêcheur',
			align    = 'top-left',
			elements = elements
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'fishing', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'fishing', vehicleProps)
		...
		local elements = {
			{label = 'Camion Poissonnerie',  value = 'fishingcar1'},
		}
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.BoatSpawnPoint.Pos, 180.0, function(vehicle)
						ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.BoatSpawnPoint.Pos, 40.00, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
				end)
			end,
...
function OpenMobileFishingActionsMenu()
							...
							if amount == nil or amount <= 0 then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									Citizen.CreateThread(function()
										...
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_fishing', 'Poissonnerie', amount)
									end)
								end
							end
						end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_fishingjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_fishingjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_fishingjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_fishingjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_fishingjob:hasEnteredMarker', function(zone)
	if zone == 'FishingFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'FishDead' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement_fish')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'FishChips' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFishChips' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'FishingActions' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
		CurrentAction     = 'fishing_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end

	if zone == 'BoatSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
		CurrentAction     = 'boat_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
	...
	if zone == 'BoatDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
...
AddEventHandler('esx_fishingjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'FishingFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
		TriggerServerEvent('esx_fishingjob:stopHarvest')
	end  
	if (zone == 'FishDead' or zone == 'FishChips') and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
		TriggerServerEvent('esx_fishingjob:stopTransform')
	end
	if (zone == 'SellFishChips') and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
		TriggerServerEvent('esx_fishingjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.FishingActions.Pos.x, Config.Zones.FishingActions.Pos.y, Config.Zones.FishingActions.Pos.z)
		SetBlipSprite (blip, 609)
		...
		AddTextComponentString("Poissonnerie")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then

		for k,v in pairs(Config.Zones)do
			if v.Type == 1 then
				local blip2 = AddBlipForCoord(v.Pos.x, v.Pos.y, v.Pos.z)

				SetBlipSprite (blip2, 609)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'fishing' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_fishingjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_fishingjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_fishingjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_fishingjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'vine_traitement' then
					TriggerServerEvent('esx_fishingjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_fishingjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'fishing_actions_menu' then
					OpenFishingActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'fishing', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'fishing' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileFishingActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)