...
TriggerEvent('esx_phone:registerNumber', 'bikerdealer', _U('dealer_customers'), false, false)
TriggerEvent('esx_society:registerSociety', 'bikerdealer', _U('car_dealer'), 'society_bikershop', 'society_bikershop', 'society_bikershop', {type = 'private'})
...
MySQL.ready(function()
	Categories     = MySQL.Sync.fetchAll('SELECT * FROM bikedeal_categories')
	local vehicles = MySQL.Sync.fetchAll('SELECT * FROM bikedeal')
...
AddEventHandler('esx_bikershop:sellVehicle', function (vehicle)
	MySQL.Async.fetchAll('SELECT * FROM bikedeal_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		['@vehicle'] = vehicle
	}, function (result)
		local id = result[1].id

		MySQL.Async.execute('DELETE FROM bikedeal_vehicles WHERE id = @id', {
			['@id'] = id
		})
	end)
end)
...
AddEventHandler('esx_bikershop:addToList', function(target, model, plate)
	...
	if xPlayer.job.name ~= 'bikerdealer' then
		print(('esx_bikershop: %s attempted to add a sold vehicle to list!'):format(xPlayer.identifier))
		return
	end

	MySQL.Async.execute('INSERT INTO bikedeal_sold (client, model, plate, soldby, date) VALUES (@client, @model, @plate, @soldby, @date)', {
...
ESX.RegisterServerCallback('esx_bikershop:getSoldVehicles', function (source, cb)
	...
	MySQL.Async.fetchAll('SELECT * FROM bikedeal_sold', {}, function(result)
		cb(result)
	end)
end)
...
AddEventHandler('esx_bikershop:getStockItem', function (itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_bikershop', function (inventory)
...
ESX.RegisterServerCallback('esx_bikershop:buyVehicleSociety', function (source, cb, society, vehicleModel)
	...
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_bikershop' --[[.. society]], function (account)
		if account.money >= vehicleData.price then
			account.removeMoney(vehicleData.price)

			MySQL.Async.execute('INSERT INTO bikedeal_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
...
ESX.RegisterServerCallback('esx_bikershop:getCommercialVehicles', function (source, cb)
	MySQL.Async.fetchAll('SELECT * FROM bikedeal_vehicles ORDER BY vehicle ASC', {}, function (result)
		local vehicles = {}
...
AddEventHandler('esx_bikershop:returnProvider', function(vehicleModel)
	...
	MySQL.Async.fetchAll('SELECT * FROM bikedeal_vehicles WHERE vehicle = @vehicle LIMIT 1', {
		...
		if result[1] then
			...
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_bikershop', function(account)
			...
			TriggerClientEvent('esx:showNotification', _source, _U('bikedeal_sold_for', vehicleModel, ESX.Math.GroupDigits(price)))
		else
