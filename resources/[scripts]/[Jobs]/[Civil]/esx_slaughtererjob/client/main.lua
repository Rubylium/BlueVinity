	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('slaughterer_clothes'), value = 'slaughterer_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'slaughterer_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'slaughterer_actions_menu'
...
function OpenSlaughtererActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'slaughterer_actions',
		{
			title    = 'Abatteur',
			align    = 'top-left',
			elements = elements
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 90.0, function(vehicle)
						...
						SetVehicleNumberPlateText(playerVeh, 'SLAU WORK')
					end)

					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'slaughterer', vehicleProps)
		...
		local elements = {
			{label = 'Camion Abbateur',  value = 'slaughterercar1'},
		}
...
function OpenMobileSlaughtererActionsMenu()
							...
							if amount == nil or amount <= 0 then
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
									...
									Citizen.CreateThread(function()
										...
										TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_slaughterer', 'Poissonnerie', amount)
									end)
								end
							end
						end,
...
AddEventHandler('esx_slaughtererjob:hasEnteredMarker', function(zone)
	if zone == 'FarmChicken' and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer'  then
		CurrentAction     = 'raisin_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'ConditChicken' and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer'  then
		CurrentAction     = 'vine_traitement'
		CurrentActionMsg  = _U('press_traitement_chicken')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'ChickenDead' and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer'  then
		CurrentAction     = 'jus_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'ChickenSell' and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'SlaughtererActions' and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' then
		CurrentAction     = 'SlaughtererActions'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' then
...
AddEventHandler('esx_slaughtererjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'FarmChicken') and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' then
		TriggerServerEvent('esx_slaughtererjob:stopHarvest')
	end  
	if (zone == 'ConditChicken' or zone == 'ChickenDead') and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' then
		TriggerServerEvent('esx_slaughtererjob:stopTransform')
	end
	if (zone == 'ChickenSell') and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' then
		TriggerServerEvent('esx_slaughtererjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.SlaughtererActions.Pos.x, Config.Zones.SlaughtererActions.Pos.y, Config.Zones.SlaughtererActions.Pos.z)
		...
		AddTextComponentString("Abatteur")
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'raisin_harvest' then
					TriggerServerEvent('esx_slaughtererjob:startHarvest', CurrentActionData.zone)
					--LoadAnimDict("givetake1_a")
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_slaughtererjob:startTransform', CurrentActionData.zone)
					--LoadAnimDict("givetake1_a")
				end
				...
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_slaughtererjob:startSell', CurrentActionData.zone)
					--LoadAnimDict("givetake1_a")
				end
				
				if CurrentAction == 'slaughterer_actions_menu' then
					OpenSlaughtererActionsMenu()
				end
				if CurrentAction == 'vehicle_spawner_menu' then
					OpenVehicleSpawnerMenu()
				end
				if CurrentAction == 'SlaughtererActions' then
					OpenCloakroomMenu()
				end
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'slaughterer', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'slaughterer' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileSlaughtererActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)