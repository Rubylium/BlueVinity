...
function SendDistressSignal()
	...
	TriggerServerEvent('esx_ambulance:Notification', x, y, z, name)
end

RegisterNetEvent('esx_ambulancejob:Notif2')
AddEventHandler('esx_ambulancejob:Notif2', function(blipId, x, y, z)
	Nombreinter = Nombreinter + 1
	if Nombreinter < 0 then
		Nombreinter = 0
	end
	Citizen.Wait(10)
	ESX.ShowAdvancedNotification('EMS INFO', 'Signale de détresse', 'Une personne à effectué un appel de détresse.\n~g~'..Nombreinter..' intervention en cours.', 'CHAR_CALL911', 1)
	...
	SetBlipScale(blipsRenfort, 1.0) -- set scale
	...
	while alpha ~= 0 do
		Citizen.Wait(10)
		alpha = alpha - 1
		--print('Alpha du blips de appel de détresse : '..alpha) --( Juste un débug )
		SetBlipAlpha(blipsRenfort, alpha)
